package com.microservicios1.shoppingService.controller;
...
public class InvoiceController {
    ...
    public ResponseEntity<Invoice> createInvoice(@Valid @RequestBody Invoice invoice, BindingResult result) {
        log.info("Creando invoice : {}", invoice);
        if (result.hasErrors()){
    ...
    public ResponseEntity<?> updateInvoice(@PathVariable("id") long id, @RequestBody Invoice invoice) {
        log.info("Actualizando Invoice con id {}", id);
        invoice.setId(id);
        Invoice invoiceActualizado=invoiceService.updateInvoice(invoice);

        if (invoiceActualizado == null) {
            return  ResponseEntity.notFound().build();
        }
        return  ResponseEntity.ok(invoiceActualizado);
    }
    @DeleteMapping("/{id}")
    public ResponseEntity<Invoice> deleteInvoice(@PathVariable("id") long id) {
        Invoice invoice = invoiceService.getInvoice(id);
        if (invoice == null) {
            log.error("Invoice no encontrado, no se puede eliminar el invoice {}.", id);
            return  ResponseEntity.notFound().build();
        }
